...
export default class App extends React.Component {
  ...
  async getWeather() {
    ...
    this.setState({
      ...
      currentTemperature: response.weather.currently.temperature, // the current temperature (e.g. 50.85)
      currentSummary: response.weather.currently.summary,     // the current weather summary (e.g. "Clear throughout the day")
      currentIcon: response.weather.currently.icon,        // the Font Awesome string of the current icon (e.g. "sun-o")
  ...
  render() {
    ...
    for (let i=0; i<5; i++){
     forecast.push(
       <View style={styles.forecastDay} key={i}>
         <Text style={styles.forecastIcon}>{this.state.forecast[i] && (<Icon size={40} name={icon(this.state.forecast[i].icon)} />)}</Text>
